<?php
	include_once("./_head.php");
	$pp = "pay_qr";
?>

<section class="container" id="pay">
	<section class="contents">
		<section class="inner">
			<!--<div class="heading-tit">
				<h2>결제방식</h2>
				<p>페이앱에서 이용가능한 다양한 결제방법</p>
			</div>-->
			<script>
				// tab group
				function tabFixed() {
					var st = $(window).scrollTop(),
						payH = $('#pay .main-section').offset().top -76; // header Height

					if( st > payH ) {
						$(".tab-group").addClass("fixed");
					} else {
						$(".tab-group").removeClass("fixed");
					}
				}
				$(document).ready(function(){ tabFixed(); });
				$(window).scroll(function(){ tabFixed(); });
				$('.tab-group .tab-menu .active').ready(function(){
					//$('.tab-group').scrollLeft($('.tab-group .tab-menu .active').position().left);
					$('.tab-group').stop().animate({scrollLeft:$('.tab-group .tab-menu .active').position().left},1000);
				});
			</script>
			<?php
				include_once("./_menu_pay.php");
			?>
		</section>

		<section class="main-section">
			<div class="inner-lg">
				<div class="title-wrap">
					<div class="heading-main">
						<div class="label-wrap">
							<label class="label label-round label-aquablue"><b>APP</b></label>
							<label class="label label-round label-violetblue"><b>PC</b></label>
						</div>
						<h2>QR코드만 부착하면<br>어디서든 결제완료</h2>
						<p>블로그, 전단지, 홍보책자, 메뉴판 등<br><span class="text-blue">다양한 곳에서 QR코드로 결제가 가능</span>합니다.</p>
					</div>
					<div class="tag-area">
						<span>#QR코드생성</span>
						<span>#어디서나</span>
						<span>#간편하게</span>
					</div>
				</div>
				<div class="video-wrap">
					<article class="video-area">
						<video loop="" autoplay="" muted="" playsinline="">
							<!-- 800*690 -->
							<source src="https://payapp.kr/homepage/images/video/mov_pay_qr.mp4" type="video/mp4">
						</video>
					</article>
				</div>
			</div>
		</section>
		<!-- //main-section -->
		<section class="use-section">
			<div class="inner">
				<div class="heading-sub">
					<h2>QR코드결제 사용방법</h2>
				</div>
				<div class="guide-slider">
					<ol class="guide-area">
						<li class="col-3">
							<strong><b>1</b>QR코드 등록</strong>
							<img src="https://payapp.kr/homepage/images/pay/qr_sec2_guide1.gif" alt="QR코드 등록 화면">
						</li>
						<li class="col-3">
							<strong><b>2</b>생성확인</strong>
							<img src="https://payapp.kr/homepage/images/pay/qr_sec2_guide2.gif" alt="생성확인 화면">
						</li>
						<li class="col-3">
							<strong><b>3</b>QR코드 배포</strong>
							<img src="https://payapp.kr/homepage/images/pay/qr_sec2_guide3.gif" alt="QR코드 배포 화면">
						</li>
						<li class="col-3">
							<strong><b>4</b>스캔후 결제</strong>
							<img src="https://payapp.kr/homepage/images/pay/qr_sec2_guide4.gif" alt="스캔후 결제 화면">
						</li>
					</ol>
				</div>
			</div>
		</section>
	</section>
</section>

<?php
	include_once("./_foot.php");
?>