<?php
	...
	if($payments == "lucy") { // 루시
		...
		$mid = $member_pg['pg_key1'];			// 루시 MID
		$apiKey = $member_pg['pg_key2'];		// 루시 apiKey

		// 1. 토큰 발급 함수
		function getToken($mid, $apikey) {
			$url = "https://lucypayments.com/api/pay/ready";
			$data = [
				"MID" => $mid,
				"APIKEY" => $apikey
			];
		...
		$API_BASE = "https://lucypayments.com/api/pay/card";	// API URL
		...
		// 2. 결제 요청 함수
		function requestPayment($token, $mid) {
			$url = "https://lucypayments.com/api/pay/card";
		...
		// 3. 실행
		$token = getToken($mid, $apikey);
		$response = requestPayment($token, $mid);
		...
		*/
		...
		if($card_result_code == "200") {
			...
			$sql_common = " stn_resultCode = '".$row['resultCode']."',
							...
							stn_payType = '".$row['data']['payType']."',
							stn_tranTime = '".$row['data']['tranTime']."',
							stn_installment = '".$row['data']['installment']."',
							stn_amount = '".$row['data']['amount']."',
							stn_applNo = '".$row['data']['applNo']."',
							stn_issueCompanyNo = '".$row['data']['issueCompanyNo']."',
							stn_issueCompanyName = '".$row['data']['issueCompanyName']."',
							stn_issueCardName = '".$row['data']['issueCardName']."',
							stn_acqCompanyNo = '".$row['data']['acqCompanyNo']."',
							stn_acqCompanyName = '".$row['data']['acqCompanyName']."' ";
	...
	if($payments == "stnb") { // 섹타나인
		...
		*/
		...
		$mbrNo = $member_pg['pg_key1'];			// 섹타나인 상점ID
		$apiKey = $member_pg['pg_key2'];		// 섹타나인 apiKey
