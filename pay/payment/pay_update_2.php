...
//		$trackId = date("Ymdhis").rand(1,9); // 주문번호
	...
	if($payments == "welcom") { // 웰컴
		...
		require('_paysis_config.php');                										// 가맹점 설정정보 정의
		...
		*/
		...
		$mbrRefNo = $order_no = "WNP".$mb_id.time();		//가맹점주문번호 (가맹점에서 생성한 중복되지 않는 번호)
		...
		$timestamp = makeTimestamp($timestamp);										// 타임스탬프 (YYYYMMDDHHMI24SS 형식의 문자열)
		...
		*/
		...
		$jsondata = serialize($row);
		$last4 = substr($card_num, -4);
		$creates = $row['data']['tranDate'].$row['data']['tranTime'];
