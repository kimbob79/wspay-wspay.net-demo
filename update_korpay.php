<?php
	...
	if(!$is_admin) {
		...
		$pg_id = $_GET['pg_id'];
		$row = sql_fetch("select * from g5_payment_korpay where pg_id = '{$pg_id}'");
		...
		$canAmt = isset($row['canAmt']) ? trim($row['canAmt']) : '';				// 취소금액
		$partCanFlg = isset($row['partCanFlg']) ? trim($row['partCanFlg']) : '';	// 부분취소여부
		$connCD = isset($row['connCD']) ? trim($row['connCD']) : '';				// 단말기/수기결제 구분
		$usePointAmt = isset($row['usePointAmt']) ? trim($row['usePointAmt']) : '';	// 카드사 사용포인트
		$vacntNo = isset($row['vacntNo']) ? trim($row['vacntNo']) : '';				// 가상계좌 번호
		$socHpNo = isset($row['socHpNo']) ? trim($row['socHpNo']) : '';				// 휴대폰번호
		...
		if($payMethod) {
			...
			if($cancelYN == "Y") {
				$pay_type = "N"; // 취소로 변경
				$tid2 = $tid;
				$pay_cdatetime =  date("Y-m-d H:i:s", strtotime($ccDnt)); // 취소날짜 날짜형식으로 변경
				$amt = "-".$amt; // 음수로 변경
				sql_query("update g5_payment set pay_cdatetime = '{$pay_cdatetime}' where trxId = '{$tid2}'"); // 기존 승인건 취소날짜 입력
			}
			...
			$pay = sql_fetch("select * from g5_payment where trxid = '{$tid}' and pay_num = '{$appNo}' and pay_type = '{$pay_type}'"); // 기존에 등록되어 있는지 확인
