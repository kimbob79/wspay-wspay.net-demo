<?php
	...
	if ($fr_date == "all" && $to_date == "all") {
		$sql_search = " where ".$adm_sql." and mb_6_name != '' and pay_type = 'N' ";
	} else {
		$sql_search = " where ".$adm_sql." and (pay_datetime BETWEEN '{$fr_dates} 00:00:00' and '{$to_dates} 23:59:59')  and mb_6_name != '' and pay_type = 'N' ";
	}
	...
	*/
	...
	$sql .= ", sum(if(dv_type = '2' and pay_type = 'N', pay, null)) as sugic";
	$sql .= ", sum(if(dv_type = '1' and pay_type = 'Y', pay, null)) as dans";
	$sql .= ", sum(if(dv_type = '1' and pay_type = 'N', pay, null)) as danc";
				...
				<tr>
					...
					<td style="text-align:center; min-width:145px;">
						<div class="buttons">
							...
							<?php if($row['pay_type'] == "N") { if($row['deposit'] == "1") { ?>
							<button  class="btn_b btn_b03" onclick="deposit_payment('<?php echo $row['pay_id'];?>','0')" type="button">입금취소설정</button>
							<?php } else { ?>
							<button  class="btn_b btn_b06" onclick="deposit_payment('<?php echo $row['pay_id'];?>','1')" type="button">입금설정</button>
							<?php } } ?>
							<?php } ?>
						</div>
					</td>
