<?php
	...
	if(!$is_admin) {
		if($type  == "2") {
			if(!$is_admin) {
				alert("권한이 없습니다. 최고관리자에게 문의해주시기 바랍니다.");
			}
		} else if($type <= $member['mb_type']) {
			alert("권한이 없습니다..", "./");
		}
	}
...
<style>
.KDC_Table__root__Jim4z  td {white-space: normal;}
</style>
...
<form name="fmember" id="fmember" action="./?p=member_form_update" onsubmit="return fregisterform_submit(this);" method="post" enctype="multipart/form-data">
	...
	<div class="KDC_Table__root__Jim4z">
		<table style="width:100%;">
			<tbody>
				...
				<?php if($mb_level < 8) { ?>
				<tr>
					<th><label for="mb_id">소속</label></th>
					<td>
						...
						<div class="select">
							<select name="mb_1" id="mb_1">
								...
								<?php
									$sql2 = " select * from g5_member where mb_level = '8'".$u_sql." order by mb_nick asc ";
						...
						<div class="select" style="margin-top:5px;">
							<select name="mb_2" id="mb_2">
								...
								<?php
									$sql2 = " select * from g5_member where mb_level = '7'".$u_sql." order by mb_nick asc ";
									$result2 = sql_query($sql2);
									for ($i=0; $row2=sql_fetch_array($result2); $i++) {
								?>
								<option value="<?php echo $row2['mb_id'] ?>" <?php if($row2['mb_id'] == $mb['mb_2']) { echo "selected"; } ?>><?php echo $row2['mb_nick']; ?></option>
								<?php
									}
								?>
							</select>
						...
						<div class="select" style="margin-top:5px;">
							<select name="mb_3" id="mb_3">
								...
								<?php
									$sql2 = " select * from g5_member where mb_level = '6'".$u_sql." order by mb_nick asc ";
									$result2 = sql_query($sql2);
									for ($i=0; $row2=sql_fetch_array($result2); $i++) {
								?>
								<option value="<?php echo $row2['mb_id'] ?>" <?php if($row2['mb_id'] == $mb['mb_3']) { echo "selected"; } ?>><?php echo $row2['mb_nick']; ?></option>
								<?php
									}
								?>
							</select>
						...
						<div class="select" style="margin-top:5px;">
							<select name="mb_4" id="mb_4">
								...
								<?php
									$sql2 = " select * from g5_member where mb_level = '5'".$u_sql." order by mb_nick asc ";
									$result2 = sql_query($sql2);
									for ($i=0; $row2=sql_fetch_array($result2); $i++) {
								?>
								<option value="<?php echo $row2['mb_id'] ?>" <?php if($row2['mb_id'] == $mb['mb_4']) { echo "selected"; } ?>><?php echo $row2['mb_nick']; ?></option>
								<?php
									}
								?>
							</select>
						...
						<?php if($mb_level <= 3) { ?>
						<div class="select" style="margin-top:5px;">
							<select name="mb_5" id="mb_5">
								<option value="">= 영업점 선택 =</option>
								<?php
									$sql2 = " select * from g5_member where mb_level = '4'".$u_sql." order by mb_nick asc ";
									$result2 = sql_query($sql2);
									for ($i=0; $row2=sql_fetch_array($result2); $i++) {
								?>
								<option value="<?php echo $row2['mb_id'] ?>" <?php if($row2['mb_id'] == $mb['mb_5']) { echo "selected"; } ?>><?php echo $row2['mb_nick']; ?></option>
								<?php
									}
								?>
							</select>
				...
				<?php /*
				<input name="apiId./v2/user/me.query.property_keys" type="text" class="KDC_Input__root__3M8Hf" placeholder="Property 키 목록, JSON Array를[&quot;properties.nickname&quot;]과 같은 형식 사용" value="">
				*/ ?>

				<tr>
					<th><label for="mb_id">아이디<?php echo $sound_only ?></label></th>
					<td style="text-align:left">
						<?php
							if($w == "u") {
						?>
							<input type="text" <?php if($mb_id) { echo "readonly"; } ?> name="mb_id" value="<?php echo $mb['mb_id']; ?>" required class="KDC_Input__root__3M8Hf" style="max-width:200px">
						<?php
							} else {
						...
						<div class="control is-expanded has-icons-left">
							...
							<?php
								/*
								$action != "new" ? "a" : "b";
								action 변수가 new 가 아닐경우에 a 맞을경우에 b
								$aaa > "1" ? "b" : "a";
								*/
							?>
				...
				<tr>
					<th><label for="mb_password">비밀번호<?php echo $sound_only ?></label></th>
					<td style="text-align:left">
						<input type="text" autocomplete="on" name="mb_password" <?php if(!$mb_id) { ?><?php } ?> class="KDC_Input__root__3M8Hf" value="<?php if($w != "u") { echo $uidpass; } ?>" style="max-width:200px">
					</td>
				</tr>
				<tr>
					<th><label for="mb_homepage">수수료</label></th>
					<td style="text-align:left"><input type="text" name="mb_homepage" value="<?php echo $mb['mb_homepage'] ?>" id="mb_homepage" required class="KDC_Input__root__3M8Hf" style="max-width:100px"> %</td>
				</tr>
				<tr>
					<th><label for="mb_nick">상호명</label></th>
					<td style="text-align:left"><input type="text" name="mb_nick" value="<?php echo $mb['mb_nick'] ?>" id="mb_nick" required class="KDC_Input__root__3M8Hf" style="max-width:200px"></td>
				</tr>
				
				<tr>
					<th><label for="mb_level">사업자등록번호</label></th>
					<td style="text-align:left">
						<input type="text" autocomplete="on" name="mb_company_number1" value="<?php echo $mb_company_number1; ?>" class="KDC_Input__root__3M8Hf" style="max-width:80px" maxlength="3" oninput="this.value = this.value.replace(/[^0-9]/g, '');">
						<input type="text" autocomplete="on" name="mb_company_number2" value="<?php echo $mb_company_number2; ?>" class="KDC_Input__root__3M8Hf" style="max-width:50px" maxlength="2" oninput="this.value = this.value.replace(/[^0-9]/g, '');">
						<input type="text" autocomplete="on" name="mb_company_number3" value="<?php echo $mb_company_number3; ?>" class="KDC_Input__root__3M8Hf" style="max-width:100px" maxlength="5" oninput="this.value = this.value.replace(/[^0-9]/g, '');">
					</td>
				</tr>
				<tr>
					<th><label for="mb_name">대표자명</label></th>
					<td style="text-align:left"><input type="text" name="mb_name" value="<?php echo $mb['mb_name'] ?>" id="mb_name" class="KDC_Input__root__3M8Hf" style="max-width:100px"></td>
				</tr>
				<tr>
					...
					<td style="text-align:left">
						...
						<input type="text" autocomplete="on" name="mb_tel2" value="<?php echo $mb_tel2; ?>" <?php if($basic['mb_tel']) { ?>required<?php } ?> class="KDC_Input__root__3M8Hf" size="4" maxlength="4" oninput="this.value = this.value.replace(/[^0-9]/g, '');" style="max-width:60px">
						<input type="text" autocomplete="on" name="mb_tel3" value="<?php echo $mb_tel3; ?>" <?php if($basic['mb_tel']) { ?>required<?php } ?> class="KDC_Input__root__3M8Hf" size="4" maxlength="4" oninput="this.value = this.value.replace(/[^0-9]/g, '');" style="max-width:60px">
					</td>
				</tr>
				<tr>
					<th>휴대전화</th>
					<td style="text-align:left">
						...
						<input type="text" autocomplete="on" name="mb_hp2" value="<?php echo $mb_hp2; ?>" class="KDC_Input__root__3M8Hf" size="4" maxlength="4" oninput="this.value = this.value.replace(/[^0-9]/g, '');" style="max-width:60px">
						<input type="text" autocomplete="on" name="mb_hp3" value="<?php echo $mb_hp3; ?>" class="KDC_Input__root__3M8Hf" size="4" maxlength="4" oninput="this.value = this.value.replace(/[^0-9]/g, '');" style="max-width:60px">
					</td>
				</tr>
				<tr>
					<th><label for="mb_email">이메일</label></th>
					<td style="text-align:left"><input type="text" name="mb_email" value="<?php echo $mb['mb_email'] ?>" id="mb_email" maxlength="100" class="KDC_Input__root__3M8Hf email" style="max-width:200px"></td>
				</tr>
				<tr>
					<th>주소</th>
					<td style="text-align:left">
						<button type="button" class="KDC_Button__root__N26ep KDC_Button__mini__Sy8ka KDC_Button__color_cancel__TdcOV" onclick="win_zip('fmember', 'mb_zip', 'mb_addr1', 'mb_addr2', 'mb_addr3', 'mb_addr_jibeon');" style="margin-bottom:2px;">주소 검색</button><br>
						<input type="text" name="mb_zip" value="<?php echo $mb['mb_zip1'] . $mb['mb_zip2']; ?>" id="mb_zip" class="KDC_Input__root__3M8Hf readonly" size="5" maxlength="6" style="max-width:200px;margin-bottom:2px;">
						<br>
						<input type="text" name="mb_addr1" value="<?php echo $mb['mb_addr1'] ?>" id="mb_addr1" class="KDC_Input__root__3M8Hf readonly" placeholder="기본주소" style="max-width:300px;margin-bottom:2px;">
						<br>
						<input type="text" name="mb_addr2" value="<?php echo $mb['mb_addr2'] ?>" id="mb_addr2" class="KDC_Input__root__3M8Hf" placeholder="상세주소" style="max-width:300px;margin-bottom:2px;">
						<br>
						<input type="text" name="mb_addr3" value="<?php echo $mb['mb_addr3'] ?>" id="mb_addr3" class="KDC_Input__root__3M8Hf" placeholder="참고항목" style="max-width:300px">
				...
				<tr>
					...
					<td style="text-align:left">
						<input type="text" autocomplete="on" name="mb_memo_call" value="<?php echo $mb['mb_memo_call']; ?>" class="KDC_Input__root__3M8Hf" style="max-width:500px" placeholder="여기 메모가 있으면 정산조회에 계좌보다 메모가 보여집니다.">
					</td>
				</tr>
				<tr>
					<th><label for="mb_8">계좌정보</label></th>
					<td style="text-align:left">
						<input type="text" autocomplete="on" name="mb_8" value="<?php echo $mb['mb_8']; ?>" class="KDC_Input__root__3M8Hf" style="max-width:100px" placeholder="은행명">
						<input type="text" autocomplete="on" name="mb_9" value="<?php echo $mb['mb_9']; ?>" class="KDC_Input__root__3M8Hf" style="max-width:200px" placeholder="계좌번호(숫자만)" oninput="this.value = this.value.replace(/[^0-9]/g, '');">
						<input type="text" autocomplete="on" name="mb_10" value="<?php echo $mb['mb_10']; ?>" class="KDC_Input__root__3M8Hf" style="max-width:100px" placeholder="예금주명">
					</td>
				</tr>
