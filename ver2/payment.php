		...
		<form id="fsearch" name="fsearch" class="local_sch01 local_sch" method="get">
			<input type="hidden" name="p" value="<?php echo $p; ?>">
				...
				<div class="KDC_Tab__root__h2hVQ tab type_other input_date search_br">
					<input type="text" name="fr_date" value="<?php echo $fr_date ?>" id="fr_date" class="KDC_Input__root__3M8Hf KDC_Input__root__3M8Hf_date" maxlength="10">
				</div>
				<div class="KDC_Tab__root__h2hVQ tab type_other input_date">
					<input type="text" name="to_date" value="<?php echo $to_date ?>" id="to_date" class="KDC_Input__root__3M8Hf KDC_Input__root__3M8Hf_date" maxlength="10">
				</div>
				<?php if($is_admin) { ?>
				<div class="KDC_Tab__root__h2hVQ tab type_other input_date" style="width:80px;">
					<select name="expansion" id="expansion" style="border:0;background:#fff;width:90%;">
						<option value="">미확장</option>
				...
				<div class="KDC_Tab__root__h2hVQ tab tab_selected type_other"><a href="./payment.xls.php?&fr_date=<?php echo $fr_date; ?>&to_date=<?php echo $to_date; ?>&expansion=<?php echo $expansion; ?>&sfl=<?php echo $sfl; ?>&stx=<?php echo $stx; ?>" class="KDC_Tab__text__VzW9X" style="background:#444; width:100%; border:0; color:#fff;"><span><i class="fa fa-file-excel-o" aria-hidden="true"></i> 엑셀출력</span></a></div>
				<?php /*
		...
		<div class="KDC_Column__root__NK8XY KDC_Column__flex_1__UcocY">
			<div class="KDC_Section__root__VXHOv">
				...
				<tr<?php echo $bgcolor?" style='background: $bgcolor;'":'';?>>
					...
					<td class="td_pay"><?php if($row['pay_cdatetime'] > 0) { echo "<del>"; } ?><?php echo number_format($row['pay']); ?><?php if($row['pay_cdatetime'] > 0) { echo "</del>"; } ?></td>
					<?php if($is_admin) { ?>
					<?php if($expansion == "Y") { ?>
					<td style="text-align:left; width:130px;"><?php if($row['mb_1_fee'] > 0) { echo "<div style='float:left;background:#333; color:#fff; font-size:11px; font-weight:100; padding:0 3px;'>".$row['mb_1_fee']."%</div><div style='float:right'>".number_format($row['mb_1_pay'])."</div>"; } ?></td>
					<td style="text-align:left; width:130px;"><?php if($row['mb_2_fee'] > 0) { echo "<div style='float:left;background:#333; color:#fff; font-size:11px; font-weight:100; padding:0 3px;'>".$row['mb_2_fee']."%</div><div style='float:right'>".number_format($row['mb_2_pay'])."</div>"; } ?></td>
					<td style="text-align:left; width:130px;"><?php if($row['mb_3_fee'] > 0) { echo "<div style='float:left;background:#333; color:#fff; font-size:11px; font-weight:100; padding:0 3px;'>".$row['mb_3_fee']."%</div><div style='float:right'>".number_format($row['mb_3_pay'])."</div>"; } ?></td>
					<td style="text-align:left; width:130px;"><?php if($row['mb_4_fee'] > 0) { echo "<div style='float:left;background:#333; color:#fff; font-size:11px; font-weight:100; padding:0 3px;'>".$row['mb_4_fee']."%</div><div style='float:right'>".number_format($row['mb_4_pay'])."</div>"; } ?></td>
					<td style="text-align:left; width:130px;"><?php if($row['mb_5_fee'] > 0) { echo "<div style='float:left;background:#333; color:#fff; font-size:11px; font-weight:100; padding:0 3px;'>".$row['mb_5_fee']."%</div><div style='float:right'>".number_format($row['mb_5_pay'])."</div>"; } ?></td>
					<td style="text-align:left; width:130px;"><?php if($row['mb_6_fee'] > 0) { echo "<div style='float:left;background:#333; color:#fff; font-size:11px; font-weight:100; padding:0 3px;'>".$row['mb_6_fee']."%</div><div style='float:right'>".number_format($row['mb_6_pay'])."</div>"; } ?></td>
					<?php } } ?>
...
function receiptPopup(id, num) {
	PopupCenter('./receipt.php?id='+id+'&num='+num, 'receipt_popup', 480, 765, {toolbar:0, resizable:0, location:0, menubar:0, status:0});
}

function payment_new(trxid, trackId, pay_id) {
	PopupCenter('./payment_new.php?trxid='+trxid+'&trackId='+trackId+'&pay_id='+pay_id, 'payment_new', 380, 805, {toolbar:0, resizable:0, location:0, menubar:0, status:0});
}
